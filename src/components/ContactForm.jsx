...
export default function ContactForm({ t: tProp, formName = 'contact' }) {
  ...
  <div className={`cform__msg ${status === 'success' ? 'cform__msg--success' : 'cform__msg--error'}`}>
    <div className="cform__msg-content">
      {status === 'success' ? (
      ...
      <span>{status === 'success' ? t.formSuccess : t.formError}</span>
    </div>
    <button className="cform__msg-close" onClick={() => setStatus('idle')} aria-label="Close">
